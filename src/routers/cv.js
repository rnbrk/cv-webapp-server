...
const { createPDFDocDefinition, styles, fonts } = require('../export/pdf');
...
router.get('/cvs/:id/pdf', async (req, res) => {
  try {
    const cv = await CV.findById(req.params.id).populate('user');
    const cvObject = cv.toJSON();

    console.dir(cvObject, { depth: null });

    const doc = createPDFDocDefinition(cvObject, styles);
